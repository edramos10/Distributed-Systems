...
int main (int argc, char *argv[]){
	...
	float **arrayA = malloc(sizeof(int*)*N);
	for(i=0;i<N;i++){
		arrayA[i] = malloc(N*sizeof(int));
	}

	float **arrayB = malloc(sizeof(int*)*N);
	for(i=0;i<N;i++){
		arrayB[i] = malloc(N*sizeof(int));
	}
	
	float **resultArray = malloc(sizeof(int*)*N);
	for(i=0;i<N;i++){
	...
	for(i=0;i<N;i++){
		for(j=0;j<N;j++){
			arrayA[i][j] = (float)rand() / RAND_MAX;//1;
			arrayB[i][j] = (float)rand() / RAND_MAX;//1;
		}
	}
